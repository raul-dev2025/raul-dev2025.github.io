...
``NAME``
    Coincidencia de nombre para la interfase de red. Podrá ser utilizada, una vez la *clave nombre* haya sido configurada en una regla precedente.

``SYMLINK``
    Coincidencia de nombre para un *symlink -enlace simbólico*, apuntando al nodo. Podrá ser utilizado una vez el ``SYMLINK`` haya sido establecido en una regla precedente. Podrían haber múltiples enlaces s; sólo es neceasrio que coincida uno.
...
``ATTR{filename}, SYSCTL{kernel parmeter}``
    Coincidencia de los valores del atributo de ``sysfs`` para el evento de dispositivo. Espacios en blanco concatenados en los valores del atributo, serán ignorados a menos que el valor de coincidencia en sí mismo, arrastre espacios en blanco. Coincidencia para un valor de *parámetro de kernel*.
