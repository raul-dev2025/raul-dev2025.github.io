...
- *Cadenas de texto* (terminación nula[f1]) serán representadas con doble comilla ``"``: string-property = “una cadena”;
...
- Procesador de *bus* local, acoplado a la *memoria mapeada* del puerto en serie, controlador de *bus spi\ *\ `f2 <f2>`__, controlador *i2c*, controlador de interrupción y, *bus puente externo(external bus bridge)*.
...
- El nodo *flash*, posée dos cadenas en la propiedad ``compatible``. Leer la siguiente
sección, para entender por qué.
- Como fué mencionado anteriormente, los nombres de nodo reflejan el tipo de dispo- sitivo, no el modelo particular. Ver sección *2.2.2* de la specificación ePAPR, para una lista genérica de nombre de nodo que deberían utilizarse habitualmente.
...
.. admonition:: nota d.t.
   ...
   * `índice <https://docs.raulvilchez.org/>`_
