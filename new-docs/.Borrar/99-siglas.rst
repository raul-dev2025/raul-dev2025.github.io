...
1.  `Referencias Generales <#i1>`__
    ...
    1.2 `Placas <#i1i2>`__
2.  `Referencias Memorias/chips <#i2>`__
...
HID – Human Interface Device, it take INPUTS and give OUTPUTS. `hardware/hid <hid.md>`__
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
xHCI – Extensible Host Controller Interface `hardware/driver/xHCI <hardware/driver/xHCI.md>`__
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. _mips-millions-of-instructions-per-second-1:
...
NUMA – Non Uniform Memory Acces (policy to alocate memory areas)`hardware/procesador <numa.md>`_
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
APM – Advanced Power Management `hardware/apm <apm.md>`__
---------------------------------------------------------
...
.. _api-application-programming-interface-1:
...
.. list-table:: Glosario ACPI
   ...
   * - Sigla
     - Descripción
   * - BGRT
     - Boot Graphics Resource Table. Tabla de recursos de gráficos de arranque.
   * - DSL
     - Domain Specific Language (en contexto ACPI, el ASL desensamblado).
   * - OSPM
...
| `CodeGuro <https://www.codeguru.com/>`__
