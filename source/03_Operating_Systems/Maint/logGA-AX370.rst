...
.. code-block:: text
    ...
    2		1
    <on-ON> -- 1. Main BIOS(Boot from the main BIOS). Activa **posición 1**.

Led asociado: M_BIOS(MBIOS_LED) **ON**
Led asociado: B_BIOS(BBIOS_LED) **off**

.. code-block:: text

    2		1
    <ON-on> -- 2. Backup BIOS(Boot from the backup BIOS). Activa **posición 2**.

Led asociado: 
Led asociado: M_BIOS(MBIOS_LED) **off**
Led asociado: B_BIOS(BBIOS_LED) **ON**

ErP, descripción
================

WARINING
Consumo de la energía remanente durante el apgado del sistema(shutdown) S5. Ver *acpi y estados de energía del sistema*.
