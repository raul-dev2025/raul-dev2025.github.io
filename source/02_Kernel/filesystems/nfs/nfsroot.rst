...
.. note:: el kernel no sincronizará la *hora* del sistema con ningún servidor NTP descubierto; ya que es responsabilidad de un proceso en el *espacio de usuario* -ejemplo, un escrito -script ``initrd/initramfs``, que pase la dirección IP listada en ``/proc/net/ipconfig/ntp_servers`` a un cliente NTP, antes de montar el FS raíz real sobre NFS.
